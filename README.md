...
SELECT 
    productos.nombre,
    SUM(compras_productos.cantidad) AS unidades_vendidas,
    SUM(compras_productos.total) AS ingreso_total
FROM miscompras.compras_productos
JOIN miscompras.productos USING (id_producto)
...
-- 2
SELECT 
    ROUND(AVG(total),2) AS promedio_total_pagado,
    ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY total),2) AS mediana_total_pagado
FROM (
...
SELECT 
    ...
    SUM(compras_productos.total) AS gasto_total,
    COUNT(DISTINCT compras.id_compra) AS numero_compras,
    RANK() OVER (ORDER BY SUM(compras_productos.total) DESC) AS ranking_gasto
FROM miscompras.compras
JOIN miscompras.clientes USING (id_cliente)
...
WITH compras_dia AS (
    SELECT 
        fecha::date AS dia,
        COUNT(DISTINCT id_compra) AS numero_compras,
...
SELECT 
    ...
    ROUND(SUM(compras_productos.total),2) AS total_categoria,
    ROUND(SUM(compras_productos.total)*100.0 / SUM(SUM(compras_productos.total)) OVER (),2) AS porcentaje
FROM miscompras.compras_productos
JOIN miscompras.productos USING (id_producto)
JOIN miscompras.categorias USING (id_categoria)
...
SELECT 
    ...
    cantidad_stock,
    CASE 
        WHEN cantidad_stock < 50 THEN 'CRITICO'
...
FROM miscompras.compras
JOIN miscompras.compras_productos USING (id_compra)
GROUP BY id_cliente, id_compra, fecha
ORDER BY id_cliente, fecha DESC;

-- 11
SELECT id_categoria, nombre, total_vendido
FROM (
    SELECT 
        productos.id_categoria,
        productos.nombre,
        SUM(compras_productos.cantidad) AS total_vendido,
        ROW_NUMBER() OVER (PARTITION BY productos.id_categoria ORDER BY SUM(compras_productos.cantidad) DESC) AS posicion
    FROM miscompras.compras_productos
    JOIN miscompras.productos USING (id_producto)
    GROUP BY productos.id_categoria, productos.nombre
) sub
WHERE posicion <= 2;
...
WHERE EXISTS (
    SELECT 1
    FROM miscompras.compras_productos cp
    JOIN miscompras.productos p ON p.id_producto = cp.id_producto
...
SELECT 
    nombre,
    ROUND(((precio_venta - (precio_venta*0.7)) / precio_venta)*100,1) AS margen_porcentual
FROM miscompras.productos;
...
SET cantidad_stock = GREATEST(p.cantidad_stock - COALESCE(v.cantidad_vendida,0),0)
FROM (
...
SELECT 
    ...
    nombre,
    precio_venta,
    DENSE_RANK() OVER (PARTITION BY id_categoria ORDER BY precio_venta DESC) AS posicion_precio
FROM miscompras.productos;
...
WITH gastos AS (
    ...
    FROM miscompras.compras
    JOIN miscompras.compras_productos USING (id_compra)
    GROUP BY id_cliente, fecha::date
)
SELECT 
    id_cliente,
    dia,
